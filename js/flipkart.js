...
for (let i = 0; i < catagory.length; i++) {
    ...
    const str = catagory[i].charAt(0).toUpperCase() + catagory[i].slice(1);
    ele.innerHTML = `
        <div class="container-fluid">
            <div class="p4-perent">
                ...
                <div class="d-flex p3-placeholder" id="${catagory[i]}">
                </div>
            </div>
        </div>`
    footer.insertAdjacentElement('beforebegin', ele);
    let placeholder = document.getElementById(catagory[i]);
    for (let j = 0; j < products.length; j++) {
        const element = products[j];
        if(element["catagory"]==catagory[i])
        {
